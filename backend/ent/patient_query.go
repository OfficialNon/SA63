...
// QueryPatientDiagnose chains the current query on the patient_diagnose edge.
func (pq *PatientQuery) QueryPatientDiagnose() *DiagnoseQuery {
	query := &DiagnoseQuery{config: pq.config}
...
//  WithPatientDiagnose tells the query-builder to eager-loads the nodes that are connected to
// the "patient_diagnose" edge. The optional arguments used to configure the query builder of the edge.
func (pq *PatientQuery) WithPatientDiagnose(opts ...func(*DiagnoseQuery)) *PatientQuery {
	query := &DiagnoseQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withPatientDiagnose = query
	return pq
}
...
func (pq *PatientQuery) sqlAll(ctx context.Context) ([]*Patient, error) {
	...
	if query := pq.withPatientDiagnose; query != nil {
		...
		query.Where(predicate.Diagnose(func(s *sql.Selector) {
			s.Where(sql.InValues(patient.PatientDiagnoseColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.patient_patient_diagnose
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "patient_patient_diagnose" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "patient_patient_diagnose" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.PatientDiagnose = append(node.Edges.PatientDiagnose, n)
		}
	}
