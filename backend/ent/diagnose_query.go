...
type DiagnoseQuery struct {
	...
	// eager-loading edges.
	withSymptoms *SymptomsQuery
	withIllness  *IllnessQuery
...
// QuerySymptoms chains the current query on the symptoms edge.
func (dq *DiagnoseQuery) QuerySymptoms() *SymptomsQuery {
	query := &SymptomsQuery{config: dq.config}
...
// QueryIllness chains the current query on the illness edge.
func (dq *DiagnoseQuery) QueryIllness() *IllnessQuery {
	query := &IllnessQuery{config: dq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := dq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
//  WithSymptoms tells the query-builder to eager-loads the nodes that are connected to
// the "symptoms" edge. The optional arguments used to configure the query builder of the edge.
func (dq *DiagnoseQuery) WithSymptoms(opts ...func(*SymptomsQuery)) *DiagnoseQuery {
	query := &SymptomsQuery{config: dq.config}
	for _, opt := range opts {
		opt(query)
	}
	dq.withSymptoms = query
	return dq
}

//  WithIllness tells the query-builder to eager-loads the nodes that are connected to
// the "illness" edge. The optional arguments used to configure the query builder of the edge.
func (dq *DiagnoseQuery) WithIllness(opts ...func(*IllnessQuery)) *DiagnoseQuery {
	query := &IllnessQuery{config: dq.config}
	for _, opt := range opts {
		opt(query)
	}
	dq.withIllness = query
	return dq
}
...
func (dq *DiagnoseQuery) sqlAll(ctx context.Context) ([]*Diagnose, error) {
	...
	if dq.withSymptoms != nil || dq.withIllness != nil || dq.withPatient != nil || dq.withDoctor != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := dq.withSymptoms; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Diagnose)
		for i := range nodes {
			if fk := nodes[i].symptoms_symptoms_diagnose; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(symptoms.IDIn(ids...))
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "symptoms_symptoms_diagnose" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Symptoms = n
			}
		}
	}

	if query := dq.withIllness; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Diagnose)
		for i := range nodes {
			if fk := nodes[i].illness_illness_diagnose; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(illness.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "illness_illness_diagnose" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Illness = n
			}
		}
	}
	...
	if query := dq.withPatient; query != nil {
		...
		nodeids := make(map[int][]*Diagnose)
		for i := range nodes {
			if fk := nodes[i].patient_patient_diagnose; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "patient_patient_diagnose" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := dq.withDoctor; query != nil {
		...
		nodeids := make(map[int][]*Diagnose)
		for i := range nodes {
			if fk := nodes[i].doctor_doctor_diagnose; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "doctor_doctor_diagnose" returned %v`, n.ID)
			}
			for i := range nodes {
