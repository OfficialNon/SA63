...
func (dq *DoctorQuery) sqlAll(ctx context.Context) ([]*Doctor, error) {
	...
	if query := dq.withDoctorDiagnose; query != nil {
		...
		for _, n := range neighbors {
			fk := n.doctor_doctor_diagnose
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "doctor_doctor_diagnose" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "doctor_doctor_diagnose" returned %v for node %v`, *fk, n.ID)
			}
